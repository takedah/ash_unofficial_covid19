...
from ash_unofficial_covid19.scrapers.child_reservation_status import ScrapeChildReservationStatus


@pytest.fixture()
def html_content():
    return """
<table id="tablepress-11-no-2" class="tablepress tablepress-id-1">
<thead>
<tr class="row-1 odd">
...
def test_lists(html_content, mocker):
    ...
    scraper = ScrapeChildReservationStatus("http://dummy.local")
    expect = [
        {
            "area": "西地区",
...
def test_get_medical_institution_list(html_content, mocker):
    ...
    mocker.patch.object(requests, "get", return_value=responce_mock)
    scraper = ScrapeChildReservationStatus("http://dummy.local")
    expect = [("旭川赤十字病院", "", "曙1条1丁目"), ("独立行政法人国立病院機構旭川医療センター", "", "花咲町7丁目")]
