...
def test_find_no_outpatient(service):
    results = service.find(medical_institution_name="おうみや内科クリニック")
...
def test_find_pediatrics(service):
    results = service.find(is_pediatrics=True)
    assert results.items[0].medical_institution_name == "市立旭川病院"
    assert len(results.items) == 1


def test_find_no_pediatrics(service):
    results = service.find(is_pediatrics=False)
    assert results.items[0].medical_institution_name == "JA北海道厚生連旭川厚生病院"
    assert len(results.items) == 2


def test_find_target_not_family(service):
    results = service.find(is_target_not_family=True)
    assert results.items[0].medical_institution_name == "JA北海道厚生連旭川厚生病院"
    assert len(results.items) == 1


def test_find_no_target_not_family(service):
    results = service.find(is_target_not_family=False)
    assert results.items[0].medical_institution_name == "旭川赤十字病院"
    assert results.items[1].medical_institution_name == "市立旭川病院"
    assert len(results.items) == 2


def test_find_medical_institution_name_and_is_pediatrics(service):
    results = service.find(medical_institution_name="旭川赤十字病院", is_pediatrics=False)
    assert results.items[0].medical_institution_name == "旭川赤十字病院"
    assert len(results.items) == 1


def test_find_is_pediatrics_and_is_target_not_family(service):
    results = service.find(is_pediatrics=True, is_target_not_family=False)
    assert results.items[0].medical_institution_name == "市立旭川病院"
    assert len(results.items) == 1
