...
class TokyoPatientsNumberService(Service):
    """東京都の新型コロナウイルス感染症日別新規患者数のデータを扱うサービス"""
    ...
    def __init__(self, pool: ConnectionPool):
        ...
        """
        Service.__init__(self, "tokyo_patients_numbers", pool)

    def create(self, tokyo_patients_numbers: TokyoPatientsNumberFactory) -> None:
        """データベースへ東京都の新型コロナウイルス感染症日別新規患者数のデータを保存

        Args:
            tokyo_patients_numbers (:obj:`TokyoPatientsNumberFactory`): 患者数データ
                東京都の新型コロナウイルス感染症日別新規患者数のデータのオブジェクトの
                リストを要素に持つオブジェクト
        ...
        data_lists = list()
        for tokyo_patients_number in tokyo_patients_numbers.items:
            data_lists.append(
                [
                    tokyo_patients_number.publication_date,
                    tokyo_patients_number.patients_number,
    ...
    def find_all(self) -> TokyoPatientsNumberFactory:
        """東京都の新型コロナウイルス感染症日別新規患者数のデータの全件リストを返す

        Returns:
            res (:obj:`TokyoPatientsNumberFactory`): 日別新規患者数のデータ一覧
                東京都の新型コロナウイルス感染症日別新規患者数データのオブジェクトの
                リストを要素に持つオブジェクト
    ...
    def get_per_hundred_thousand_population_per_week(self, from_date: date, to_date: date) -> list:
        ...
        for aggregate in aggregate_by_weeks:
            ...
            per_hundred_thousand_population = float(
                Decimal(str(patients_number / Config.TOKYO_POPULATION * 100000)).quantize(
    ...
    def get_last_update_date(self) -> date:
        """東京都のオープンデータの最新の公表日を取得する

        Returns:
            last_update_date (date): 東京都のオープンデータの最新の公表日を取得する
