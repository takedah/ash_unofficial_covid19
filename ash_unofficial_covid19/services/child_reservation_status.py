...
from ..models.child_reservation_status import ChildReservationStatusFactory, ChildReservationStatusLocationFactory
from ..services.database import ConnectionPool
from ..services.service import Service


class ChildReservationStatusService(Service):
    ...
    def __init__(self, pool: ConnectionPool):
        ...
        """
        table_name = "child_reservation_statuses"
        Service.__init__(self, table_name, pool)

    def create(self, child_reservation_statuses: ChildReservationStatusFactory) -> None:
        """データベースへ新型コロナワクチン接種医療機関の予約受付状況データを保存

        Args:
            child_reservation_statuses (:obj:`ChildReservationStatusFactory`): 予約受付状況データ
        ...
        data_lists = list()
        for reservation_status in child_reservation_statuses.items:
            data_lists.append(
                [
    ...
    def find(
        ...
        Returns:
            results (list of :obj:`ChildReservationStatusLocation`): 予約受付状況詳細データ
        ...
        factory = ChildReservationStatusLocationFactory()
        with self.get_connection() as cur:
            if len(search_args) == 0:
