...
class ScrapeTokyoPatientsNumber(Scraper):
    """東京都の新型コロナウイルス感染症日別患者数データの抽出

    東京都オープンデータカタログサイトからダウンロードした陽性患者数CSVファイルから、
    東京都の新型コロナウイルス感染症日別患者数データを抽出し、リストに変換する。
    ...
    def _extract_patients_number_data(self, row: list) -> Optional[dict]:
        ...
        Returns:
            patients_number_data (dict): 東京都の日別患者数データを表すハッシュ
